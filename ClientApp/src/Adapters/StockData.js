...
//Current Price URL: https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=IBM&entitlement=delayed&apikey=CBVMME6BACI5A2Y6
//Daily Price History (15min interval): https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=IBM&interval=15min&entitlement=delayed&apikey=CBVMME6BACI5A2Y6
...
export async function getCurrenPrice(ticker) {
    ...
    if (timeDiff > 0) {
        ...
        const url = `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${ticker}&entitlement=delayed&apikey=CBVMME6BACI5A2Y6`
        try {
...
export async function getDailyPriceHistory(ticker) {
    ...
    if (timeDiff > 0) {
        ...
        const url = `https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=${ticker}&interval=15min&entitlement=delayed&apikey=CBVMME6BACI5A2Y6`
        try {
            const response = await fetch(url, {
                headers: {
                    "Content-Type": "application/json",
